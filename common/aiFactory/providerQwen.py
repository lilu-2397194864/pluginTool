...
class QwenAPITester(AIProviderInterface):
    """通义千问API测试器"""
    
    def __init__(self, api_key: str = None):
        """初始化Qwen API测试器"""
        self.logger = Logger('QWEN_API_TEST')
        if api_key:
            dashscope.api_key = api_key
        
        self.vision_keywords = ['vl', 'vision', 'ocr']
        self.chat_keywords = ['qwen', 'chat']
        self.embedding_keywords = ['text-embedding']
        
        self.default_qwen_chat_model = "qwen-max"
        self.default_qwen_vision_model = "qwen-vl-max"
        self.default_temperature = 0.7
        self.default_max_length = 500
        
        self.ocr_detail_levels = {
            'simple': 'Extract the text in the image. Do not add any other content.',
...
请直接输出 Markdown 内容，不要添加额外的解释。"""
        ...
        self.available_models = [
            ...
            {'name': 'qwen-turbo', 'model': 'qwen-turbo'},
    ...
    def _get_model(self, model_name: str = None, model_type: str = None):
        ...
        if model_name:
            for model in self.available_models:
                if model['model'] == model_name or model['name'] == model_name:
        ...
        if model_type == 'vision':
            default_model = self.default_qwen_vision_model
        else:
            default_model = self.default_qwen_chat_model
    ...
    def list_models(self, verbose: bool = True) -> List[Dict[str, Any]]:
        """列出可用模型"""
        self.logger.context(logging.INFO, 'Start testing list available models API')
    ...
    def show_model(self, model_name: str = None, verbose: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if verbose:
                ...
                self.logger.context(logging.INFO, f'Model: {target_model}')
                self.logger.context(logging.INFO, f'Capabilities: {model_info.get("capabilities", "Unknown")}')
                self.logger.context(logging.INFO, f'Max tokens: {model_info.get("max_tokens", "Unknown")}')
    ...
    def chat_completion(self, model_name: str = None, prompt: str = None,
        ...
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                self.logger.context(logging.ERROR, f'Chat completion failed: {response.code}, {response.message}')
                return None
        ...
        try:
            ...
            for response in responses:
                if response.status_code == HTTPStatus.OK:
                    ...
                    self.logger.context(logging.ERROR, f'Streaming response error: {response.code}, {response.message}')
                    break
    ...
    def generate_text(self, model_name: str = None, prompt: str = None,
        ...
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                self.logger.context(logging.ERROR, f'Text generation failed: {response.code}, {response.message}')
                return None
    ...
    def create_embeddings(self, model_name: str = None, text: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                self.logger.context(logging.ERROR, f'Embedding generation failed: {response.code}, {response.message}')
                return None
    ...
    def extract_image_text(self, image_path: str, model_name: str = "qwen-vl-plus",
        ...
        try:
            ...
            if not target_model:
                ...
                self.logger.context(logging.INFO, 'Please make sure you have proper access to Qwen vision models')
                return None
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                if result_length > 0:
                    ...
                    self.logger.context(logging.ERROR, 'OCR result is empty')
                    return None
                ...
                self.logger.context(logging.ERROR, f'OCR extraction failed: {response.code}, {response.message}')
                return None
    ...
    def image_to_markdown(self, image_path: str, model_name: str = None,
        ...
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                if content_length > 0:
                    ...
                    try:
                        ...
                        if simple_response.status_code == HTTPStatus.OK:
                            ...
                            self.logger.context(logging.ERROR, f'Simple description failed: {simple_response.code}, {simple_response.message}')
                            return None
                ...
                self.logger.context(logging.ERROR, f'Markdown conversion failed: {response.code}, {response.message}')
                return None
    ...
    def describe_image(self, image_path: str, model_name: str = None,
        ...
        try:
            ...
            self.logger.context(logging.DEBUG, f'Calling multimodal conversation API for image description, detail level: {detail_level}')
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                self.logger.context(logging.ERROR, f'Image description failed: {response.code}, {response.message}')
                return None
    ...
    def analyze_chart(self, image_path: str, model_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.context(logging.DEBUG, 'Calling multimodal conversation API for chart analysis')
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                self.logger.context(logging.ERROR, f'Chart analysis failed: {response.code}, {response.message}')
                return None
    ...
    def check_health(self) -> bool:
        """检查服务健康状态"""
        self.logger.context(logging.INFO, 'Start testing network connectivity')
        
        try:
            # Test network connectivity by making a simple HTTP request
            response = requests.get("https://www.aliyun.com", timeout=10)
            self.logger.context(logging.INFO, f'Network connectivity check completed, status code: {response.status_code}')
            
            if response.status_code == 200:
                self.logger.context(logging.INFO, 'Network is accessible')
            else:
                self.logger.context(logging.WARN, f'Network is accessible but got unexpected status: {response.status_code}')
            
            return response.status_code == 200
            
        except requests.exceptions.ConnectionError:
            self.logger.context(logging.ERROR, 'Network is not accessible, please check internet connection')
            return False
        except Exception as e:
